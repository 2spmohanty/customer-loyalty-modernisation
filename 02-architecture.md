...
| From | To | Why |
|------|----|-----|
| 45+ table JOIN per request | Single document fetch | One read returns the complete customer profile — no JOIN required |
| Single overloaded RDBMS | MongoDB Atlas sharded cluster | 32M records distributed across shards — 5X peak handled by design not by over-provisioning |
| No domain isolation | Three clean collections | Profile, Loyalty, and Compliance separated at the storage layer |
| EU data ungoverned | Atlas Global Cluster with zone pinning | GDPR enforced by architecture — not by application logic |
| No observability strategy | Atlas Performance Advisor built in | p95 latency monitored continuously — no additional tooling required |
